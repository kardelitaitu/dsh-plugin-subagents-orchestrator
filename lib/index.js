...
function extractFallbackChain(config) {
  if (config && Array.isArray(config.fallback) && config.fallback.length > 0) {
    return config.fallback.filter((e) => Boolean(e && e.provider && e.model && e.enabled !== false));
  }
  return [];
}
...
function recordTurnSuccess(agentId, tokens, now = Date.now()) {
  ...
  successSpans.delete(agentId);
  if (!span.failed) {
    const effective = tokens !== void 0 ? tokens : span.pendingTokens;
...
function sampleSuccess(agentId, endpoint, span, now, tokens) {
  ...
  if (tokens !== void 0) entry.tokensTotal += Math.max(0, tokens);
  emit({
    at: now,
    type: "success",
    agentId,
...
function buildFailoverNotice(info) {
  ...
  const reason = info.code !== void 0 && info.hintMs !== void 0 ? `${info.code}, cooldown hint ${info.hintMs}ms` : info.code ?? (info.hintMs !== void 0 ? `cooldown hint ${info.hintMs}ms` : "connection failure");
...
function apply(ctx) {
  ...
  const disposeRequest = ctx.on("agent/request", async (payload, next) => {
    ...
    const endpoints = getCachedEndpoints();
    const target = current.tier === "fallback" ? getCachedFallbackChain()[current.index] : endpoints[current.index];
